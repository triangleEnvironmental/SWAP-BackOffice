      ...
      >
        <template v-slot:actions>
          <BaseButton
            ...
            color="info"
            :route-name="route('service-provider-area.list', {id: data.id})"
            :icon="Icons.area"
...
const title = computed(() => {
  if (!isEdit.value) {
    ...
    if (route().current('my-service-provider.edit')) {
...
const deleteLogo = async () => {
  const confirm = await confirmModalRef.value.openModal({
    title: 'Delete service provider logo',
    content: 'Please confirm that you need to delete the current service provider logo.'
  });
  if (confirm) {
    Inertia.delete(route('service-provider-logo.delete', {id: route().params.id}), {
...
const isEdit = computed(() => {
  return !route().current('service-provider.create')
})
...
const submit = () => {
  ...
  if (isEdit.value) {
    spForm.transform(data => ({
      ...data,
      _method: 'PUT'
    })).post(route('service-provider.update', {id: route().params.id}), {
      preserveScroll: false,
      onSuccess: () => {
      },
    });
  } else {
    spForm.post(route('service-provider.post'), {
      preserveScroll: false,
      onSuccess: () => {
        resetForm();
      },
